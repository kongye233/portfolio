...
void mainImage(out vec4 o, vec2 C){
  ...
  if(uGrainAnimated>0.5){grainUv+=vec2(iTime*0.12,iTime*-0.08);}
...
export default function Grainient({
  ...
  useEffect(() => {
    ...
    const loop = time => {
      const elapsed = elapsedBeforePause + (time - startedAt) * 0.001
      program.uniforms.iTime.value = elapsed
      render()
      frameCount += 1
      if (frameCount % 15 === 0) canvas.dataset.animationFrame = String(frameCount)
    ...
    const stop = () => {
      if (frame === 0) return
      elapsedBeforePause = program.uniforms.iTime.value
